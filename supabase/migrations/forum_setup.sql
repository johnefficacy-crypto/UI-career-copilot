...
--   1. RLS policies on all forum tables
...
-- Reputation — public read, system write only
...
BEGIN
  ...
  ELSIF TG_TABLE_NAME = 'forum_comments' THEN
    ...
    ELSIF TG_OP = 'DELETE' THEN
      UPDATE public.forum_reputation
        SET points = GREATEST(points - 1, 0), comments_count = GREATEST(comments_count - 1, 0)
        WHERE user_id = target_user;
    END IF;

  ELSIF TG_TABLE_NAME = 'forum_post_upvotes' THEN
    -- Award points to the post author (not the voter)
    SELECT user_id INTO target_user FROM public.forum_posts WHERE id = COALESCE(NEW.post_id, OLD.post_id);
    IF TG_OP = 'INSERT' THEN
