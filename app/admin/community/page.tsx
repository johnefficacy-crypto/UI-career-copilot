import { listForumReports } from "@/lib/db/forum-moderation"
  ...
  return (
    <div className="p-6 md:p-8">
      ...
      <div className="space-y-4">
        {reports.map((r) => (
          <form key={r.id} action={updateForumReportAction} className="rounded-xl border border-white/10 bg-white/[0.02] p-4">
            <input type="hidden" name="report_id" value={r.id} />
            <div className="flex flex-wrap items-center justify-between gap-3 mb-3">
              <div>
                <p className="text-xs text-white/40">{r.target_type.toUpperCase()} • {new Date(r.created_at).toLocaleString()}</p>
                <p className="text-white text-sm font-medium">{r.reason}</p>
              </div>
              <span className="text-[11px] px-2 py-1 rounded-full border border-white/15 text-white/70">{r.id.slice(0, 8)}</span>
            </div>

            <p className="text-white/60 text-xs mb-3">{r.details ?? "No additional details."}</p>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-3">
              <label className="text-xs text-white/60">Status
                <select name="status" defaultValue={r.status} className="w-full mt-1 rounded-md bg-black/40 border border-white/15 px-2 py-1.5 text-sm text-white">
                  {STATUS_OPTIONS.map((opt) => <option key={opt} value={opt}>{opt}</option>)}
                </select>
              </label>
              <label className="text-xs text-white/60">Severity
                <select name="severity" defaultValue={r.severity} className="w-full mt-1 rounded-md bg-black/40 border border-white/15 px-2 py-1.5 text-sm text-white">
              ...
              <label className="text-xs text-white/60">Target
                <div className="mt-1 rounded-md bg-black/40 border border-white/15 px-2 py-1.5 text-sm text-white/85">
                  {r.target_type === "post" ? r.post?.title ?? "Post" : (r.comment?.body ?? "Comment").slice(0, 72)}
                </div>
              </label>
            </div>

            <label className="text-xs text-white/60 block mb-3">Action notes
              <textarea name="action_notes" defaultValue={r.action_notes ?? ""} rows={2} className="w-full mt-1 rounded-md bg-black/40 border border-white/15 px-2 py-1.5 text-sm text-white" />
            </label>
...
function FilterLink({ label, href, active }: { label: string; href: string; active: boolean }) {
  return (
    <a href={href} className="rounded-lg border px-3 py-2 text-sm" style={{
