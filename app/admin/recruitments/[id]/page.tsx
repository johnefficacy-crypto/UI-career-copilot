...
// }: {
//   params: { id: string }
//   searchParams: { error?: string; post?: string }
// }) {
//   const [rec, organizations] = await Promise.all([
//     getRecruitmentById(params.id),
//     getAllOrganizations(),
//   ])
...
//   const editingPostId = searchParams.post ?? null
...
//         <span className="text-white/60 text-sm truncate">{rec.name}</span>
//       </div>
...
//                       <a
//                         href={`/admin/recruitments/${rec.id}?post=${post.id}`}
...
//                         <button
//                           type="submit"
//                           className="text-red-400/40 text-xs hover:text-red-400 transition-colors px-2 py-1"
//                         >
//                           ✕
//                         </button>
//                       </form>
...
//           {/* Post form — add new or edit existing */}
                ...
                >
                  ...
                  <div className="flex flex-wrap gap-1.5 mt-2">
                    ...
                    {post.education_criteria?.[0] && (
                      <span className="text-[10px] bg-white/[0.05] border border-white/[0.08] text-white/40 px-1.5 py-0.5 rounded">
                        {post.education_criteria[0].min_qualification_level ?? "Any edu"}
                        {post.education_criteria[0].min_percentage
                          ? ` · ${post.education_criteria[0].min_percentage}%`
                          : ""}
                      </span>
                    )}
                    ...
                    {post.attempt_limits?.length > 0 && (
                      <span className="text-[10px] bg-white/[0.05] border border-white/[0.08] text-white/40 px-1.5 py-0.5 rounded">
                        {post.attempt_limits.length} attempt limit
                        {post.attempt_limits.length > 1 ? "s" : ""}
                      </span>
                    )}
                    ...
                    {post.vacancies?.length > 0 && (
                      <span className="text-[10px] bg-white/[0.05] border border-white/[0.08] text-white/40 px-1.5 py-0.5 rounded">
                        {post.vacancies.reduce(
                          (s: number, v) => s + (v.vacancy_count ?? 0),
                          0
                        )}{" "}
                        vacancies
                      </span>
